...
from ep_ab.datasets import SAbDabDataset
from ep_ab.models import get_model
from ep_ab.utils.misc import *
from ep_ab.utils.data import *
...
if __name__ == '__main__':
    ...
    dataset_ = functools.partial(SAbDabDataset, processed_dir=cfg.dataset.processed_dir, surface=cfg.dataset.get('surface', None), use_plm=cfg.model.get('use_plm', None),
                                 transform=get_transform(cfg.dataset.transform) if 'transform' in cfg.dataset else None, test_dir=cfg.dataset.get('test_dir', None))
    train_dataset = dataset_(split='train', relax_struct=cfg.dataset.relax_struct, pred_struct=cfg.dataset.pred_struct, )
    val_dataset = dataset_(split='val', relax_struct=cfg.dataset.relax_struct, pred_struct=cfg.dataset.pred_struct, )

    train_iterator = inf_iterator(DataLoader(train_dataset, batch_size=cfg.train.batch_size, collate_fn=PaddingCollate(max_pts=cfg.train.max_pts), shuffle=True, num_workers=args.num_workers))
    val_loader = DataLoader(val_dataset, batch_size=cfg.train.batch_size * 2, collate_fn=PaddingCollate(), shuffle=False, num_workers=args.num_workers)
            ...
            for _, batch in enumerate(tq(loader, desc='Validate', dynamic_ncols=True)):
                ...
                if 'mask' not in batch:                                                                                                    # torch geometric data does not have 'mask'
                    if 'surfformer' in cfg.model.type and cfg.train.eval_resolution == 'patch':                                            # patch-level validation
                        if not cfg.model.patch_setup.fixed_patches:
                            ...
                            pc_list = zip(batch['id'], output['ep_true_coarse'].cpu().numpy(), output['ep_pred_coarse'].cpu().numpy())         # (B, K)
                    else:
                ...
                for idx, ep_true, ep_pred in pc_list:
                    ...
                    res.append(
                        {'complex': idx, 'roc_auc': roc_auc_pc, 'au_prc': au_prc_pc, 'precision': precision_pc, 'recall': recall_pc, 'balanced_acc': balanced_acc, 'f1': f1_pc,

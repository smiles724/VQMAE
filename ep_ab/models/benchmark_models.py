...
from ep_ab.utils.geometry import dMaSIFConv, tangent_vectors
...
class SAModule(torch.nn.Module):
    ...
    def forward(self, x, pos, batch):
        ...
        # idx = fps(pos, batch, ratio=xxx.ratio)  # Extract xxx.ratio indices TURN OFF FOR NOW
...
class PointNet2_seg(torch.nn.Module):
    def __init__(self, args, in_channels, out_channels):
        ...
        # xxx.sa1_module = SAModule(1.0, xxx.radius, MLP([xxx.I+3, xxx.O, xxx.O]),xxx.k)
        ...
        self.linear_layers = nn.ModuleList([nn.Sequential(nn.Linear(self.O, self.O), nn.ReLU(), nn.Linear(self.O, self.O)) for i in range(self.n_layers)])

        self.linear_transform = nn.ModuleList([nn.Linear(self.I, self.O)] + [nn.Linear(self.O, self.O) for i in range(self.n_layers - 1)])
...
class dMaSIFConv_seg(torch.nn.Module):
    ...
    def load_mesh(self, xyz, normals=None, weights=None, ranges=None):

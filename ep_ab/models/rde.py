...
from ep_ab.modules.encoders.residue import PerResidueEncoderV1
from ep_ab.modules.encoders.pair import ResiduePairEncoder
from ep_ab.modules.encoders.ga import GAEncoder
from ep_ab.modules.flows.spline import ContextualCircularSplineFlow
from ep_ab.modules.flows.coupling import ContextualCircularSplineCouplingLayer
from ep_ab.modules.flows.container import ContextualSequentialFlow
from ep_ab.utils.protein.constants import BBHeavyAtom, num_aa_types, chi_angles_atoms
from ep_ab.modules.common.geometry import construct_3d_basis, angstrom_to_nm
...
def _build_flow(num_chis, n_context_dims, cfg_flow):
    ...
    if num_chis == 1:
        ...
        for i in range(cfg_flow.num_blocks):
            blocks.append(ContextualCircularSplineFlow(n_context_dims=n_context_dims, n_hidden_dims=cfg_flow.num_hidden_dims, n_spline_bins=cfg_flow.num_spline_bins, ))
    else:
        for i in range(cfg_flow.num_blocks):
            update_dim = [i % num_chis, ]
            blocks.append(ContextualCircularSplineCouplingLayer(n_dims=num_chis, mapping_dims=update_dim, n_context_dims=n_context_dims, n_hidden_dims=cfg_flow.num_hidden_dims,
...
class CircularSplineRotamerDensityEstimator(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Encoding
        self.single_encoder = PerResidueEncoderV1(feat_dim=cfg.blocks.node_feat_dim, max_num_atoms=5,  # N, CA, C, O, CB,
                                                  )
        self.masked_bias = nn.Embedding(num_embeddings=2, embedding_dim=cfg.blocks.node_feat_dim, padding_idx=0, )
        self.pair_encoder = ResiduePairEncoder(feat_dim=cfg.blocks.pair_feat_dim, max_num_atoms=5,  # N, CA, C, O, CB,
                                               )
        self.attn_encoder = GAEncoder(**cfg.blocks)

        # Flows
        n_context_dim = cfg.blocks.node_feat_dim
        self.flows = nn.ModuleList([_build_flow(n_chis, n_context_dim, cfg.flow) for n_chis in range(1, 4 + 1)])
    ...
    def _mle_loss(self, batch, logprobs):
        ...
        for n_chis in range(1, 4 + 1):
            logprob = logprobs[n_chis - 1]  # (N, L), joint distribution of n_chis angles
    ...
    def forward(self, batch):
        ...
        for n_chis in range(1, 4 + 1):
            z, logabsdet = self.flows[n_chis - 1](batch['chi_native'][:, :, :n_chis], c, inverse=False)
    ...
    def sample(self, batch, n_samples=1, residue_subset=None):
        ...
        zs = [sample_latent(c.shape[:-1], n_chis, device=c.device) for n_chis in range(1, 4 + 1)]  # [(N, L, 1), ..., (N, L, 4)]
        logpzs = [_latent_log_prob(zs[n_chis - 1], n_chis) for n_chis in range(1, 4 + 1)]

        xs, logprobs = [], []
        for n_chis in range(1, 4 + 1):
            x, logabsdet = self.flows[n_chis - 1](zs[n_chis - 1], c, inverse=True)  # (N, L, n_chis), (N, L)
            x = F.pad(x, pad=(0, 4 - n_chis), value=0)  # (N, L, 4)
            logpx = logpzs[n_chis - 1] + logabsdet  # (N, L)
            xs.append(x)
            logprobs.append(logpx)

        xs = F.pad(torch.stack(xs, dim=-1), pad=(1, 0), value=0)  # (N, L, 4, 5)
        logprobs = F.pad(torch.stack(logprobs, dim=-1), pad=(1, 0), value=0)  # (N, L, 5)

        n_chis_all = self.num_chis_of_aa[batch['aa'].flatten()].reshape_as(batch['aa'])  # (N, L)
        xs = torch.gather(xs, dim=-1, index=n_chis_all[:, :, None, None].expand_as(xs))[..., 0]  # (N, L, 4)
        logprobs = torch.gather(logprobs, dim=-1, index=n_chis_all[:, :, None].expand_as(logprobs))[..., 0]  # (N, L)

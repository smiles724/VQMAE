...
pyrosetta.init(' '.join(
    ['-mute', 'all', '-use_input_sc', '-ignore_unrecognized_res', '-ignore_zero_occupancy', 'false', '-load_PDB_components', 'false', '-relax_struct:default_repeats', '2',
     '-no_fconfig', ]))
...
def get_scorefxn(scorefxn_name: str):
    ...
    corrections = {'beta_july15': False, 'beta_nov16': False, 'gen_potential': False, 'restore_talaris_behavior': False, }
...
class RelaxRegion(object):
    ...
    def __call__(self, pdb_path, flexible_residue_first, flexible_residue_last):
        ...
        if self.subset != 'all':
            ...
            gen_selector.set_index_range(pose.pdb_info().pdb2pose(*flexible_residue_first), pose.pdb_info().pdb2pose(*flexible_residue_last), )
...
def run_pyrosetta(task: RelaxTask):
    ...
    pose_min, _, _ = minimizer(pdb_path=task.current_path, flexible_residue_first=task.flexible_residue_first, flexible_residue_last=task.flexible_residue_last, )

...
from src.utils.protein.constants import AA, BBHeavyAtom
...
def load_complex_structure(fname, return_map=False, ligand=None, receptor=None):
            ...
            if chain_id in ligand:
                ...
                for atom in res.get_atoms():
                    if atom.name in BBHeavyAtom._member_names_:
                        atom_types_ligand.append(BBHeavyAtom[atom.element])
                ...
                for atom in res.get_atoms():
                    if atom.name in BBHeavyAtom._member_names_:
                        ...
                        atom_types_receptor.append(BBHeavyAtom[atom.element])
                    if atom.name == 'CA':
    ...
    atom_types_ligand_array = np.zeros((len(atom_types_ligand), len(BBHeavyAtom)))  # one-hot embeddings
    for i, t in enumerate(atom_types_ligand):
        atom_types_ligand_array[i, t] = 1.0
    atom_types_receptor_array = np.zeros((len(atom_types_receptor), len(BBHeavyAtom)))  # one-hot embeddings
...
def load_structure(fname, return_map=False):
    ...
    for i, res in enumerate(residues):
        ...
        if AA.is_aa(resname) and resname != 'UNK' and (res.has_id('CA') and res.has_id('C') and res.has_id('N')):        # skip unknown residues
            ...
            for atom in res.get_atoms():
                if atom.name in BBHeavyAtom._member_names_:   # 'atom.element' and 'atom.name' are different!
                    atom_coords.append(atom.get_coord())
                    atom_types.append(BBHeavyAtom[atom.name])
                    atom_res.append(i)
                if atom.name == 'CA':
    ...
    atom_types_array = np.zeros((len(atom_types), len(BBHeavyAtom)))   # one-hot embeddings

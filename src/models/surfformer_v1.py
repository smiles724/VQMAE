...
def farthest_point_sample(xyz, npoint, mask):
    """
    https://github.com/CGuangyan-BIT/PointGPT/blob/9c56432a7b7dc4a071ef681f3ca6cd0f7babba0f/segmentation/pointnet_util.py
    Input:
        xyz: pointcloud data, [B, N, 3]
        npoint: number of samples
        mask: [B, N]
    Return:
        patches: sampled pointcloud index, [B, npoint]
    """
    device = xyz.device
    B, N, C = xyz.shape
    patches = torch.zeros(B, npoint, dtype=torch.long).to(device)
    distance = torch.ones(B, N).to(device) * 1e10
    weights = torch.ones_like(mask).float() * mask
    farthest = torch.multinomial(weights / torch.sum(weights, dim=-1, keepdim=True), num_samples=1).squeeze(-1).to(device)
    batch_indices = torch.arange(B, dtype=torch.long).to(device)
    for i in range(npoint):
        patches[:, i] = farthest
        patch = xyz[batch_indices, farthest, :].view(B, 1, 3)
        dist = torch.sum((xyz - patch) ** 2, -1)
        dist[~mask] = 1e-10
